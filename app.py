...
# 2. 侧边栏
# ==========================================
st.sidebar.header("🐼 熊猫策略 (熔断版)")
st.sidebar.info(
    f"""
    **策略逻辑：**
    1. **进攻 (QLD)**: SPY > 200MA & QQQ动量 > 0
    2. **熔断 (QQQ)**: 5天跌幅 > 7.5% (当月强制降级)
    3. **防守 (Cash)**: 信号失效
    """
)
st.sidebar.divider()
...
# 4. 主界面逻辑
# ==========================================
st.title("🐼 Panda Strategy (Full History + Circuit Breaker)")
...
if df is not None:
    ...
    with col1:
        ...
        if current_pos == 2: # QLD
            st.markdown(f"""<div class='signal-box risk-on'><h1>🎋 进攻 (BUY QLD)</h1><p>SPY趋势向上 & 动量充足</p></div>""", unsafe_allow_html=True)
        elif current_pos == 1: # QQQ (熔断)
            st.markdown(f"""<div class='signal-box meltdown'><h1>⚠️ 熔断降级 (HOLD QQQ)</h1><p><b>触发风控！</b> 短期避险模式</p></div>""", unsafe_allow_html=True)
        else: # Cash
    ...
    backtest_df['Ret_QLD_Syn'] = backtest_df['Ret_QQQ'] * 2.0 - daily_drag
    ...
    choices = [backtest_df['Ret_QLD_Syn'], backtest_df['Ret_QQQ'], backtest_df['Ret_Cash']]
    ...
    if not plot_df.empty:
        ...
        if not buy_pts.empty:
            fig.add_trace(go.Scatter(x=buy_pts.index, y=buy_pts['Strat_Cum'], mode='markers', name='Buy QLD', marker=dict(symbol='triangle-up', size=12, color='green', line=dict(width=1, color='black'))))
        ...
        if not melt_pts.empty:
            fig.add_trace(go.Scatter(x=melt_pts.index, y=melt_pts['Strat_Cum'], mode='markers', name='Meltdown (QQQ)', marker=dict(symbol='x', size=10, color='orange', line=dict(width=1, color='black'))))
