...
<style>
    ...
    /* 表格样式优化 */
    .perf-table { width: 100%; border-collapse: collapse; color: #fff; font-family: 'Share Tech Mono'; }
    .perf-table th { border-bottom: 1px solid #00ffff; color: #00ffff; padding: 10px; text-align: right; }
    ...
    .log-item { border-left: 2px solid #555; padding-left: 10px; margin-bottom: 8px; font-family: 'Share Tech Mono'; font-size: 0.9rem; }
</style>
...
def get_data_and_backtest():
    ...
    # === 回测逻辑 (Vectorized) ===
    # 1. 信号判断
    # 熊猫 Bull: SPY > MA 和 QQQ > MOM
    df['Signal_Bull'] = (df['SPY'] > df['SPY_MA']) & (df['QQQ'] > df['QQQ_MOM_Ref'])
    # 敢死队 Squad: QQQ < LowerBand 和 RSI < 30
    df['Signal_Squad'] = (df['QQQ'] < df['Lower_Band']) & (df['RSI'] < SQ_RSI_ENTRY)
    # 熔断 Meltdown: Drawdown < -CB_DROP
    df['Signal_Meltdown'] = df['Drawdown'] < -CB_DROP
    
    # 2. 策略持仓判定 (优先级: 熔断 > 敢死队 > 熊猫)
    # 仓位代码: 0=Cash, 1=QQQ(1x), 2=QLD(2x 模拟)
    conditions = [
        df['Signal_Meltdown'],  # 优先级 1: 熔断 -> 切 1x QQQ
        df['Signal_Squad'],     # 优先级 2: 敢死队 -> 抄底 2x
        df['Signal_Bull']       # 优先级 3: 牛市 -> 2x
    ]
    choices = [1, 2, 2]         # 对应仓位系数
    df['Position'] = np.select(conditions, choices, default=0) # 默认熊市空仓 (0)
    ...
    # 策略日收益 (昨天的持仓 * 今天的涨跌幅)
    # 简化模拟: 2x 杠杆 = 2 * daily_return (实际 ETF 会有损耗，这里做理论参考)
    df['Strat_Ret'] = df['Position'].shift(1) * df['QQQ_Ret']
    
    # 修正: 2x 杠杆如果是 -2% 应该是 -4%，简单乘法在大跌时会有偏差，但做理论参考足够
    # 处理空仓时的收益 (假设为 0，或可设为国债收益率，这里设为0)
    ...
    # === 状态变更记录 ===
    # 0: Cash, 1: Meltdown(1x), 2: Bull/Squad(2x)
    # 为了更细致的日志，我们标记状态名称
    status_conds = [
        df['Signal_Meltdown'],
        df['Signal_Squad'],
...
def clean_html(html_str): return "".join([line.strip() for line in html_str.split('\n') if line.strip()])
...
# ==========================================
col_perf, col_log = st.columns([1.2, 0.8])

with col_perf:
    st.markdown("""
    <div style='background: rgba(0,255,255,0.1); border-left: 4px solid #00ffff; padding: 5px 15px;'>
    ...
    st.markdown("<p style='font-family: Share Tech Mono; font-size: 0.8rem; color: #666; margin-top: 10px;'>*注: 理论回测假设 牛市/突击=2x杠杆(QLD), 熔断=1x杠杆(QQQ), 熊市=空仓(0%). 未包含交易滑点与费率.</p>", unsafe_allow_html=True)
...
with col_log:
    ...
    log_html = "<div style='margin-top: 10px; max-height: 300px; overflow-y: auto;'>"
